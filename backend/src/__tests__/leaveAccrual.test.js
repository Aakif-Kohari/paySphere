...
describe('Leave Accrual Math Engine', () => {
  ...
  describe('calculateProRatedAccrual', () => {
    ...
    it('should grant full rate if employee worked the entire month', () => {
      ...
      expect(calculateProRatedAccrual(monthlyRate, start, end, 1, 2024)).toBe(
        1.5,
      );
    });
    ...
    it('should grant 0 if employee joins after the month ends', () => {
      ...
      expect(calculateProRatedAccrual(monthlyRate, start, end, 2, 2024)).toBe(
        0,
      );
    });
    ...
    // back to the first or last day of the month when it was not — in either
    // direction. A period entirely outside the month therefore produced a
    // full month's accrual. The "joins after the month ends" case above is
    // that bug, and it has been failing on main since it was written.

    it('grants 0 for a period that starts in a later month', () => {
      const start = new Date(2026, 10, 1); // November
      const end = new Date(2026, 10, 30);

      expect(calculateProRatedAccrual(monthlyRate, start, end, 8, 2026)).toBe(
        0,
      );
    });

    it('grants 0 for a period that ended in an earlier month', () => {
      // Somebody who left in June must not accrue for August.
      const start = new Date(2020, 0, 1);
      const end = new Date(2026, 5, 15); // June
    ...
    it('grants 0 for the same month in a different year', () => {
    ...
    it('grants a full month when the period spans it end to end', () => {
      const start = new Date(2020, 0, 1);
    ...
    it('pro-rates a leaver from the start of the month to their exit', () => {
      // 1-15 August is 15 of 31 days. 1.5 * 15/31 = 0.7258 -> 0.73
      const start = new Date(2020, 0, 1);
    ...
    it.each([
      ['a zero rate', 0],
      ['a negative rate', -5],
      ['a rate that is not a number', 'lots'],
      ['an undefined rate', undefined],
    ])('grants 0 for %s', (_label, rate) => {
      // A policy with no usable accrualRate must credit nothing rather
      // than NaN — an $inc of NaN fails the write and loses the employee.
      const start = new Date(2026, 7, 1);
      const end = new Date(2026, 7, 31);

      expect(calculateProRatedAccrual(rate, start, end, 8, 2026)).toBe(0);
    });

    it('grants 0 for an unparseable date', () => {
