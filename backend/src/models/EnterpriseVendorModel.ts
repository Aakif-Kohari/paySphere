...
export function createMockVendors(): IVendor[] {
...
export function createMockPurchaseOrders(): IPurchaseOrder[] {
  return [
    {
...
export function createMockInvoices(): IInvoice[] {
  return [
    {
...
export function createMockContracts(): IProcurementContract[] {
  return [
    {
...
export function computeVendorSpendByCategory(vendors: IVendor[]): Array<{ category: ProcurementCategory; totalSpend: number; vendorCount: number }> {
  const map = new Map<ProcurementCategory, { totalSpend: number; vendorCount: number }>();
...
export function computeRiskSummary(vendors: IVendor[]): { low: number; medium: number; high: number; critical: number } {
  const summary = { low: 0, medium: 0, high: 0, critical: 0 };
  for (const v of vendors) {
    if (v.riskScore <= 15) summary.low++;
    else if (v.riskScore <= 30) summary.medium++;
    else if (v.riskScore <= 60) summary.high++;
