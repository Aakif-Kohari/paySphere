// Enterprise Compliance & Audit Trail Suite — Data Models
...
export interface ICompliancePolicy {
  id: string; name: string; category: ComplianceCategory; description: string;
  effectiveDate: string; lastReviewedAt: string; nextReviewDate: string;
  owner: string; status: 'active' | 'under_review' | 'archived';
  requirements: string[]; applicableRegions: string[];
  riskRating: SeverityLevel; attachments: string[];
}

export interface IAuditRecord {
  id: string; auditNumber: string; title: string; category: ComplianceCategory;
  auditor: string; auditDate: string; status: AuditStatus;
  scope: string; findings: Array<{ id: string; description: string; severity: SeverityLevel; recommendation: string; remediationStatus: RemediationStatus; assignedTo: string; dueDate: string }>;
  score: number | null; nextAuditDate: string; region: string;
}

export interface IComplianceIncident {
  id: string; incidentNumber: string; title: string; category: ComplianceCategory;
  severity: SeverityLevel; reportedBy: string; reportedAt: string;
  description: string; affectedRecords: number; region: string;
  status: 'open' | 'investigating' | 'contained' | 'resolved' | 'closed';
  resolutionNotes: string | null; resolvedAt: string | null;
}

export function createMockPolicies(): ICompliancePolicy[] {
  return [
...
export function createMockAudits(): IAuditRecord[] {
  return [
...
export function createMockIncidents(): IComplianceIncident[] {
  return [
...
export function computeComplianceByCategory(policies: ICompliancePolicy[]): Array<{ category: ComplianceCategory; count: number; avgRisk: string }> {
  const map = new Map<ComplianceCategory, { count: number; risks: string[] }>();
...
export function computeAuditEffectiveness(audits: IAuditRecord[]): { completedCount: number; avgScore: number; totalFindings: number; resolvedFindings: number; resolutionRate: string } {
...
export function computeIncidentSeverity(incidents: IComplianceIncident[]): { critical: number; high: number; medium: number; low: number } {
  const dist = { critical: 0, high: 0, medium: 0, low: 0 };
  for (const i of incidents) {
    if (i.severity in dist) dist[i.severity as keyof typeof dist]++;
  }
  return dist;
}

// Aggregation: policy review status
export function computePolicyReviewStatus(policies: ICompliancePolicy[]): { upToDate: number; dueSoon: number; overdue: number } {
  const now = new Date();
