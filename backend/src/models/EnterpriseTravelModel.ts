...
export interface ITravelAnalytics {
  ...
  topDestinations: Array<{ city: string; trips: number; spend: number }>;
  spendByCategory: Array<{ category: ExpenseCategory; amount: number; percentage: number }>;
  departmentSpend: Array<{ department: string; trips: number; spend: number }>;
...
export function createMockTravelRequests(): ITravelRequest[] {
  return [
    {
...
export function createMockExpenseReports(): IExpenseReport[] {
  return [
    {
...
export function createMockPerDiemRates(): IPerDiemRate[] {
  return [
...
export function computeDepartmentSpend(requests: ITravelRequest[]): Array<{ department: string; trips: number; totalSpend: number }> {
  const map = new Map<string, { trips: number; totalSpend: number }>();
...
export function computeTopDestinations(requests: ITravelRequest[]): Array<{ city: string; country: string; trips: number; totalSpend: number }> {
  const map = new Map<string, { city: string; country: string; trips: number; totalSpend: number }>();
  for (const r of requests) {
