 ...
 */
salaryHistorySchema.index({ createdAt: -1 }, { name: 'idx_created_at' });
...
salaryHistorySchema.pre('save', function (next) {
  ...
  if (this.previousSalary > 0) {
    this.percentageChange =
      Math.round(
        ((this.newSalary - this.previousSalary) / this.previousSalary) * 10000,
      ) / 100;
  } else {
...
salaryHistorySchema.plugin(softDeletePlugin);
