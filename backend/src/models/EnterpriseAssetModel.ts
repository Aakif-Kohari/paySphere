// Enterprise Asset Management & IT Inventory — Data Models
...
export interface IAsset {
  id: string; tagNumber: string; name: string; category: AssetCategory; model: string; serialNumber: string;
  purchaseDate: string; purchasePrice: number; currentValue: number; depreciationRate: number;
  status: AssetStatus; condition: Condition; assignedTo: string | null; department: string;
  location: string; office: string; warrantyExpiry: string; lastMaintenanceDate: string | null;
  nextMaintenanceDate: string | null; notes: string;
}

export interface IAssetRequest {
  id: string; requestNumber: string; employeeId: string; employeeName: string; department: string;
  assetType: AssetCategory; justification: string; priority: 'low' | 'medium' | 'high' | 'urgent';
  status: 'pending' | 'approved' | 'fulfilled' | 'denied'; requestedAt: string; fulfilledAt: string | null;
}

export interface ISoftwareLicense {
  id: string; softwareName: string; vendor: string; licenseKey: string; totalSeats: number;
  usedSeats: number; expiryDate: string; cost: number; autoRenew: boolean; status: 'active' | 'expiring' | 'expired';
}

export function createMockAssets(): IAsset[] {
  return [
...
export function createMockSoftwareLicenses(): ISoftwareLicense[] {
  return [
...
export function createMockAssetRequests(): IAssetRequest[] {
  return [
...
export function computeAssetsByCategory(assets: IAsset[]): Array<{ category: AssetCategory; count: number; totalValue: number }> {
  const map = new Map<AssetCategory, { count: number; totalValue: number }>();
...
export function computeAssetsByOffice(assets: IAsset[]): Array<{ office: string; count: number; totalValue: number }> {
...
export function computeDepreciationSummary(assets: IAsset[]): { totalPurchasePrice: number; totalCurrentValue: number; totalDepreciation: number; avgDepreciationRate: number } {
