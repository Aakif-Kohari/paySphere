 ...
 * @description Manages vendor CRUD, invoice uploads with auto-TDS, and payment recording.
 * Issue: #957
 */
const mongoose = require('mongoose');
const { Vendor, VendorInvoice, VendorPayment } = require('../models/vendor.model');
const { calculateTDS194C, isValidPAN } = require('../utils/contractorTdsCalculator');
...
exports.createVendor = async (req, res, next) => {
    try {
        ...
        const vendor = await Vendor.create({
            tenantId: req.tenantId, name, pan, gstin, vendorType, address, contactEmail
        });
 ...
 * Upload/Record a new invoice. Automatically calculates 194C TDS.
 */
exports.createInvoice = async (req, res, next) => {
    try {
        const { invoiceNumber, invoiceDate, grossAmount } = req.body;
        ...
        // Calculate TDS
        const tdsCalc = await calculateTDS194C(vendor, Number(grossAmount), financialYear, req.tenantId);
        ...
        eventBus.emit('AUDIT_LOG', {
            ...
            details: { vendor: vendor.name, grossAmount, tdsDeducted: tdsCalc.tdsAmount },
            req,
        });
...
exports.getVendorLedger = async (req, res, next) => {
    try {
        ...
        const ytdTds = invoices
            ...
            .reduce((sum, inv) => sum + inv.tdsAmount, 0);
