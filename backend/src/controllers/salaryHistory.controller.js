    ...
    try {
        ...
        res.status(200).json({
            ...
            statistics: {
                ...
                averagePercentageChange: Math.round(aggregatedStats.averagePercentageChange * 100) / 100,
...
exports.createSalaryHistoryManual = async (req, res, next) => {
    try {
        ...
        if (isNaN(prevSalaryNum) || prevSalaryNum < 0) {
            return res.status(400).json({ message: 'Previous salary must be a non-negative number' });
        }

        if (isNaN(newSalaryNum) || newSalaryNum < 1) {
            return res.status(400).json({ message: 'New salary must be a positive number' });
        }
    ...
    try {
        ...
        const csvRows = historyRecords.map((record) => {
            ...
            return [
                ...
                escapeField(changer.fullName || changer.email || record.changedByName || ''),
...
exports.deleteSalaryHistory = async (req, res, next) => {
    try {
        ...
        if (!mongoose.Types.ObjectId.isValid(id)) {
            return res.status(400).json({ message: 'Invalid salary history ID format' });
        }
        ...
        // Delete the entry
        await SalaryHistory.findByIdAndDelete(id);
...
exports.getSalaryStatistics = async (req, res, next) => {
    try {
        const monthsBack = Math.min(Math.max(parseInt(req.query.months) || 12, 1), 60);
                ...
                summary: [
                    {
                        $project: {
                            ...
                            averagePercentageChange: { $round: ['$averagePercentageChange', 2] },
                            maxChange: 1,
                            minChange: 1,
                            totalIncreases: 1,
                            totalDecreases: 1,
