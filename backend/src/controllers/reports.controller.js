...
const getAnalytics = async (req, res, next) => {
    try {
        ...
        const monthsBack = Math.min(Math.max(parseInt(query.months || '') || 6, 1), 12);
        ...
        const monthlyTrends = Object.values(monthlyMap).sort((a, b) => a.year - b.year || a.month - b.month);
        ...
        const roleBreakdown = Object.values(roleMap).sort((a, b) => b.totalPayout - a.totalPayout);
        ...
        const totalDeductions = payrolls.reduce((sum, p) => sum + p.deductions + p.leaveDeduction, 0);
...
const downloadPDFReport = async (req, res, next) => {
    try {
        ...
        const totalBonus = payrolls.reduce((sum, p) => sum + p.bonus, 0);
        const totalDeductions = payrolls.reduce((sum, p) => sum + p.deductions + p.leaveDeduction, 0);
        const pdfWorker = new worker_threads_1.Worker(path.join(__dirname, '../workers/pdf.worker.js'));
        let isHandled = false;
...
const generatePayslipBuffer = (employee, payroll, currency = 'INR') => {
    return new Promise((resolve, reject) => {
        ...
        if (bd && (bd.bankName || bd.accountNumber || bd.routingCode)) {
            ...
            doc.fontSize(10).font('Helvetica').fillColor('#555555');
            if (bd.bankName)
                doc.text(`Bank Name: ${bd.bankName}`);
            if (bd.accountNumber)
                doc.text(`Account Number: ${bd.accountNumber}`);
            if (bd.routingCode)
                doc.text(`Routing / IFSC Code: ${bd.routingCode}`);
        }
        doc.end();
    });
};
...
const exportExcelReport = async (req, res, next) => {
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename=payroll-summary-${monthName}-${year}.xlsx`);
...
const downloadPayslipsZip = async (req, res, next) => {
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename=payslips-${monthName}-${year}.zip`);
        archive.pipe(res);
...
const getTurnoverMetrics = async (req, res, next) => {
    try {
            ...
            for (const emp of allEmployees) {
                ...
                if (joinDate <= monthEnd) {
                    if (!termDate || termDate > monthEnd) {
                        ...
                        terminatedThisMonth++;
                        const tenureDays = (termDate.getTime() - joinDate.getTime()) / (1000 * 60 * 60 * 24);
        ...
        const turnoverRate = averageActiveEmployees > 0
            ...
            : 0;
        const averageTenureDays = terminatedCount > 0 ? Math.round(totalTenureDays / terminatedCount) : 0;
