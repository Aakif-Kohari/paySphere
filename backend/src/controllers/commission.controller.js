...
exports.createPlan = async (req, res, next) => {
    try {
        const plan = await CommissionPlan.create({ ...req.body, tenantId: req.tenantId });
...
exports.recordRevenue = async (req, res, next) => {
    try {
        ...
        const plan = await CommissionPlan.findOne({ tenantId: req.tenantId, isActive: true }).sort({ effectiveFrom: -1 });
        ...
        const lastLedger = await DrawLedger.findOne({ employeeId, tenantId: req.tenantId }).sort({ createdAt: -1 });
        ...
        const attainment = await QuotaAttainment.findOneAndUpdate(
            ...
            {
                tenantId: req.tenantId, planId: plan._id, revenueBooked,
                quotaTarget: plan.quotaAmount, attainmentPercentage: calc.attainment,
                calculatedCommission: drawResult.netPayout, status: 'Calculated'
            },
        ...
        if (drawResult.drawOffset > 0) {
            await DrawLedger.create({
                tenantId: req.tenantId, employeeId, transactionType: 'Commission Offset',
                amount: -drawResult.drawOffset, balanceAfter: drawResult.newDrawBalance,
                referenceId: attainment._id, description: `Offset against ${periodMonth}/${periodYear} commission`
            });
        }
...
exports.issueDrawAdvance = async (req, res, next) => {
    try {
        ...
        await DrawLedger.create({
            tenantId: req.tenantId, employeeId, transactionType: 'Draw Advance',
            amount, balanceAfter: newBalance, description: 'Monthly recoverable draw advance'
        });
...
exports.processClawback = async (req, res, next) => {
    try {
        ...
        const clawback = await Clawback.create({
            tenantId: req.tenantId, employeeId, originalAttainmentId, dealName, clawbackAmount, reason,
            status: clawbackResult.requiresPayrollDeduction ? 'Recovered via Payroll' : 'Recovered via Draw'
        });

        // Add to draw ledger
        await DrawLedger.create({
            tenantId: req.tenantId, employeeId, transactionType: 'Clawback',
            amount: clawbackAmount, balanceAfter: clawbackResult.newDrawBalance,
            referenceId: clawback._id, description: `Clawback for ${dealName} (${reason})`
        });
...
exports.getMyDashboard = async (req, res, next) => {
    try {
        ...
        const attainments = await QuotaAttainment.find({ employeeId: employee._id, tenantId: req.tenantId })
            .populate('planId', 'name quotaAmount')
            .sort({ periodYear: -1, periodMonth: -1 }).limit(12);

        const lastLedger = await DrawLedger.findOne({ employeeId: employee._id, tenantId: req.tenantId }).sort({ createdAt: -1 });
        const drawBalance = lastLedger ? lastLedger.balanceAfter : 0;

        const clawbacks = await Clawback.find({ employeeId: employee._id, tenantId: req.tenantId, status: 'Pending Recovery' });
