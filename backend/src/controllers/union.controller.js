...
exports.createCBA = async (req, res, next) => {
    try {
        const cba = await CollectiveBargainingAgreement.create({ ...req.body, tenantId: req.tenantId });
...
exports.addTier = async (req, res, next) => {
    try {
        const tier = await UnionDuesTier.create({ ...req.body, tenantId: req.tenantId });
...
exports.calculateDuesBatch = async (req, res, next) => {
    try {
        ...
        const cba = await CollectiveBargainingAgreement.findOne({ _id: cbaId, tenantId: req.tenantId });
        if (!cba) return res.status(404).json({ message: 'CBA not found' });

        const tiers = await UnionDuesTier.find({ cbaId, tenantId: req.tenantId });

        // Fetch all active employees covered by this CBA (simplified: all active employees)
        const employees = await Employee.find({ tenantId: req.tenantId, isActive: true }).select('_id fullName monthlySalary');
...
exports.fileGrievance = async (req, res, next) => {
    try {
        ...
        const grievance = await GrievanceArbitration.create({
            tenantId: req.tenantId, employeeId, cbaId: req.body.cbaId,
            title, description, filedDate: new Date(), stepDeadline: deadline
        });
...
exports.getAdminDashboard = async (req, res, next) => {
    try {
        const cbas = await CollectiveBargainingAgreement.find({ tenantId: req.tenantId }).sort({ effectiveFrom: -1 });
        const grievances = await GrievanceArbitration.find({ tenantId: req.tenantId })
            .populate('employeeId', 'fullName')
