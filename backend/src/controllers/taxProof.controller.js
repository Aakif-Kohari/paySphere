...
const { calculateTDSAdjustment, aggregateApprovedDeductions } = require('../utils/tdsAdjuster');
...
exports.submitProof = async (req, res, next) => {
    try {
        ...
        res.status(201).json({ message: 'Tax proof submitted successfully', proof });
    } catch (error) {
        if (error.code === 11000) return res.status(409).json({ message: 'Proof for this section already submitted for this FY.' });
        next(error);
    }
};
...
exports.getMyProofs = async (req, res, next) => {
    try {
        ...
        const proofs = await TaxProof.find({ employeeId: employee._id, tenantId: req.tenantId }).sort({ createdAt: -1 });
...
exports.verifyProof = async (req, res, next) => {
    try {
        ...
        const proof = await TaxProof.findById(req.params.id);

        if (!proof || proof.tenantId.toString() !== req.tenantId) {
        ...
        if (status !== 'Approved') {
            ...
            tdsAdjustment = calculateTDSAdjustment(proof.claimedAmount, proof.approvedAmount, 0.30);
        }
        ...
        eventBus.emit('AUDIT_LOG', {
            ...
            resourceType: 'TaxProof',
            resourceIds: [proof._id],
            details: { status, approvedAmount: proof.approvedAmount, tdsImpact: tdsAdjustment?.taxImpact },
            req,
        });

        res.status(200).json({ message: 'Proof verified successfully', proof, tdsAdjustment });
    } catch (error) {
        next(error);
    }
};
