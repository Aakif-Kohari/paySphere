...
const payrollWorker = new Worker("payroll-processing", async (job) => {
  let session = null;
  try {
    logger.info(`Starting payroll processing job ${job.id} for user ${job.data.userId}`);
    ...
    for (let i = 0; i < preparedItems.length; i++) {
      ...
      if (existingRecord) {
        throw new Error(`Payroll already processed for ${item.employee.fullName} for ${currentMonth}/${currentYear}`);
      }
      ...
      // Update job progress
      await job.updateProgress(Math.floor(((i + 1) / preparedItems.length) * 100));
    }
