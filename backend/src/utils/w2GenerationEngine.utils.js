...
function calculateW2Boxes(ytdData) {
    ...
    if (box1_Wages < box3_SSWages && preTaxDeductions > 0) {
        ...
        hasDiscrepancy = true;
        discrepancyNotes = 'WARNING: Box 1 exceeds Social Security wage base without valid pre-tax offsets.';
    }
...
function generateERRecord(employerData, taxYear) {
    ...
    const contactPhone = padString(employerData.contactPhone.replace(/\D/g, ''), 15);
    const contactEmail = padString(employerData.contactEmail || '', 50);
    const blank2 = padString('', 96);

    const record = recordType + taxYearStr + kind + ein + controlNumber + establishmentNumber +
        employerName + addressLine1 + addressLine2 + city + state + zip + zipExt +
        blank1 + contactName + contactPhone + contactEmail + blank2;
...
function generateRWRecord(employee, w2Data, taxYear) {
    ...
    const record = recordType + ssn + empEIN + firstName + middleInitial + lastName + suffix +
        address1 + address2 + city + state + zip + zipExt + blank1 + statEmp +
        blank2 + retirementPlan + thirdPartySickPay + blank3 +
        w1 + w2 + w3 + w4 + w5 + w6 + w7 + w8 + w9 + w10 + w11 + blank4 +
        w12a_Code + w12a_Amount + blank5;
...
function generateRORecord(totals, employeeCount) {
    ...
    const record = recordType + blank1 + empCount + w1 + w2 + w3 + w4 + w5 + w6 + w7 + w8 + w9 + w10 + w11 + blank2;
    return record.padEnd(512, ' ').substring(0, 512);
}
...
function generateRTRecord(roTotals, roRecordCount) {
    ...
    const record = recordType + blank1 + count + w1 + w2 + w3 + w4 + w5 + w6 + w7 + w8 + w9 + w10 + w11 + blank2;
    return record.padEnd(512, ' ').substring(0, 512);
}
