...
router.post('/findings/:id/remediate', (req: Request, res: Response) => {
  for (const audit of audits) {
    const finding = audit.findings.find((f) => f.id === req.params.id);
    if (finding) {
      finding.remediationStatus = req.body.status || 'in_progress';
      if (req.body.status === 'resolved') finding.remediationStatus = 'resolved';
      if (req.body.status === 'verified') finding.remediationStatus = 'verified';
