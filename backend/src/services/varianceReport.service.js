...
async function getMonthlyVariance(tenantId, year, month) {
  ...
  const prevMap = {};
  previous.forEach((d) => {
    prevMap[d.department] = d;
  });
  const budgetMap = {};
  budgets.forEach((b) => {
    budgetMap[b.department] = b;
  });
  const departments = current.map((curr) => {
    ...
    return {
      ...
      budgetVariance:
        budget.budgetedGross != null
          ? curr.totalGross - budget.budgetedGross
          : null,
    };
  });
  const totals = departments.reduce(
    (a, d) => ({
      gross: a.gross + d.current.gross,
      headcount: a.headcount + d.current.headcount,
    }),
    { gross: 0, headcount: 0 },
  );
...
async function getAnnualForecast(tenantId, year) {
  ...
  for (let m = 1; m <= 12; m++) {
    ...
    try {
      ...
      logger.error('forecast agg failed', { m, error: err.message });
    }
    ...
    if (g > 0) {
      ...
      const p = rolling.length
        ? rolling.reduce((s, v) => s + v, 0) / rolling.length
        : null;
      results.push({
        month: m,
        actual: null,
        projected: p ? Number(p.toFixed(2)) : null,
      });
    }
  }
  return results;
