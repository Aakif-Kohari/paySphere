...
import {
  createMockTravelRequests,
  createMockExpenseReports,
  createMockPerDiemRates,
  computeDepartmentSpend,
  computeTopDestinations,
} from '../models/EnterpriseTravelModel';

const router = Router();

const travelRequests = createMockTravelRequests();
const expenseReports = createMockExpenseReports();
const perDiemRates = createMockPerDiemRates();
...
router.get('/requests', (req: Request, res: Response) => {
  ...
  if (tripType) filtered = filtered.filter((r) => r.tripType === tripType);
  if (search) {
...
router.get('/expenses/:id', (req: Request, res: Response) => {
  ...
  if (!expense) return res.status(404).json({ error: 'Expense report not found' });
  res.json({ expense });
});

// GET /api/travel/per-diem — list per diem rates with optional location filter
router.get('/per-diem', (req: Request, res: Response) => {
...
router.get('/analytics', (_req: Request, res: Response) => {
  ...
  const topDestinations = computeTopDestinations(travelRequests);
  const departmentSpend = computeDepartmentSpend(travelRequests);
...
router.post('/expenses/:id/approve', (req: Request, res: Response) => {
  const expense = expenseReports.find((e) => e.id === req.params.id);
  if (!expense) return res.status(404).json({ error: 'Expense report not found' });
