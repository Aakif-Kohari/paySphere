...
describe("CacheService (#407)", () => {
  ...
  describe("Redis Mode (REDIS_URL set)", () => {
    ...
    test("should set value with TTL in Redis", async () => {
      ...
      expect(mockRedisClient.setex).toHaveBeenCalledWith("myKey", 30, JSON.stringify({ data: "test" }));
    });
    ...
    test("should invalidate pattern without RangeError on large number of keys (#407)", async () => {
      ...
      const largeKeysArray = Array.from({ length: 1200 }, (_, i) => `analytics:user1:key_${i}`);
      ...
      expect(mockRedisClient.del).toHaveBeenCalledWith(largeKeysArray.slice(0, 500));
      expect(mockRedisClient.del).toHaveBeenCalledWith(largeKeysArray.slice(500, 700));
      expect(mockRedisClient.del).toHaveBeenCalledWith(largeKeysArray.slice(700, 1200));
    });
  });
});
