...
class EnterpriseTimeAttendanceService {
    ...
    getDashboardMetrics() {
        ...
        const presentToday = this.timeEntries.filter((e) => e.status === 'ACTIVE' ||
            e.status === 'COMPLETED' ||
            e.status === 'APPROVED').length;
        ...
        const avgAttendance = totalEmployees > 0
            ? Math.round((this.attendanceRecords.reduce((s, r) => s + r.attendancePercentage, 0) /
                totalEmployees) *
                100) / 100
            : 0;
        return {
            ...
            compliantPercentage: totalEmployees > 0
