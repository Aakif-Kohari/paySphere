...
export default function AttendanceCalendarModal({
  ...
  const applyServerDays = useCallback((days) => {
    ...
    (Array.isArray(days) ? days : []).forEach((entry) => {
      next[entry.day] = {
        ...
        isGazettedHoliday: Boolean(entry.isGazettedHoliday),
      };
    });
    setDayStates(next);
  }, []);
  ...
  const handleTileClick = (dayNum) => {
    setDayStates((prev) => {
      ...
      let allowedStatuses = STATUS_KEYS;
      if (current.isGazettedHoliday) {
        allowedStatuses = ['HOLIDAY', 'OVERTIME']; // Restrict absences on statutory holidays
      }

      const currentIndex = allowedStatuses.indexOf(current.status);
      const nextIndex = (currentIndex + 1) % allowedStatuses.length;
      const nextStatus = allowedStatuses[nextIndex] || 'HOLIDAY';
      ...
      return {
        ...prev,
        [dayNum]: { ...current, status: nextStatus, otHours: newOtHours },
      };
    });
  };
  ...
  const handleMarkAll = (targetStatus) => {
    setDayStates((prev) => {
      ...
      for (let d = 1; d <= daysInMonth; d++) {
        ...
        if (dateObj.getDay() !== 0) {
          // Keep Sundays as Paid Leave
          updated[d] = {
            status: targetStatus,
            otHours: targetStatus === 'OVERTIME' ? 2 : 0,
          };
        }
      }
      return updated;
  ...
  const handleSaveOt = () => {
    if (selectedDayForOt) {
      setDayStates((prev) => ({
        ...
        [selectedDayForOt]: {
          status: 'OVERTIME',
          otHours: Math.min(24, Math.max(0.5, parseFloat(otInput) || 0)),
        },
      }));
      ...
      >
        ...
        >
          <div>
            ...
            >
              Cycle tiles to log attendance for {monthName} {year}. Click day
              tile to change status.
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        {(loading || loadError || saveError || isLocked) && (
          ...
          >
            ...
            {isLocked && (
              ...
              >
                🔒 This month is locked — its payroll has been paid and the
        ...
        <div style={{ padding: '20px 24px', overflowY: 'auto', flex: 1 }}>
          ...
          >
              ...
              (dayName, idx) => (
                <div
                  ...
                  style={{
                    ...
                    fontWeight: 700,
                    color:
                      idx === 0 ? '#EF4444' : isDark ? '#9CA3AF' : '#6B7280',
            ...
            {Array.from({ length: daysInMonth }).map((_, idx) => {
              ...
              const dayState = dayStates[d] || {
                status: 'PRESENT',
                otHours: 0,
              };
              const cfg =
                STATUS_CONFIG[dayState.status] || STATUS_CONFIG.PRESENT;
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            ...
            <button
              ...
              style={{
                ...
                fontWeight: 700,
                cursor:
                  saving || loading || isLocked ? 'not-allowed' : 'pointer',
