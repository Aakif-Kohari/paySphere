...
export default function PayrollTable({ data = [], currency = 'INR' }) {
  ...
  if (!data || data.length === 0) {
    return (
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-12 text-center">
        <p className="text-sm text-gray-500 dark:text-slate-400">No payroll records available for this period.</p>
      </div>
    );
  }
  ...
  const renderStatusBadge = (status) => {
    const statusConfig = {
      ...
      pending_approval: { bg: 'bg-yellow-100 dark:bg-yellow-900/30', text: 'text-yellow-800 dark:text-yellow-300', label: 'Pending' },
      rejected: { bg: 'bg-red-100 dark:bg-red-900/30', text: 'text-red-800 dark:text-red-300', label: 'Rejected' },
      finalized: { bg: 'bg-purple-100 dark:bg-purple-900/30', text: 'text-purple-800 dark:text-purple-300', label: 'Finalized' }
    };

    const config = statusConfig[status?.toLowerCase()] || statusConfig.pending_approval;
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700 table-fixed">
          ...
          <thead className="sticky top-0 z-10 bg-gray-50 dark:bg-slate-900 shadow-[inset_0_-1px_0_rgba(0,0,0,0.1)] dark:shadow-[inset_0_-1px_0_rgba(255,255,255,0.1)]">
            <tr>
              ...
              <th scope="col" className="px-6 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-slate-400">
                Base Salary
              </th>
              <th scope="col" className="px-6 py-3 text-right text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-slate-400">
                Overtime
              ...
              <th scope="col" className="px-6 py-3 text-center text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-slate-400">
                Status
              </th>
            </tr>
          </thead>
