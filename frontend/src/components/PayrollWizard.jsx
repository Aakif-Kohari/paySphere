...
export default function PayrollWizard() {
  ...
  useEffect(() => {
    const handleBeforeUnload = (e) => {
      if (!showSuccess && step > 1) {
        e.preventDefault();
        e.returnValue = "Are you sure you want to discard your payroll wizard progress?";
      }
    };
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      setFinalizeError(
        err.response?.data?.message || "Failed to finalize payroll. Please try again."
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const handleSendEmails = async () => {
    ...
    try {
      ...
      setEmailMsg(res.data.message || "Payslip emails successfully sent to all employees!");
      setEmailStatus("success");
    } catch (err) {
      setEmailMsg(
        err.response?.data?.message || "Failed to send payslip emails. Please try again."
      );
      ...
      >
        <div style={{ textAlign: "center", marginBottom: 30 }}>
          ...
          <p style={{ fontSize: 14, color: isDark ? "#9CA3AF" : "#6B7280" }}>
            {payrollResults.message || "Payroll run has been finalized successfully!"}
          </p>
        </div>

        {/* Individual Net Salaries */}
        <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 30 }}>
          <h3 style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.05em", textTransform: "uppercase", color: isDark ? "#9CA3AF" : "#6B7280" }}>
            ...
            {resultsList.map((res, i) => (
              <div
                ...
                style={{
                  ...
                  padding: "16px",
                  borderBottom: i < resultsList.length - 1 ? (isDark ? "1px solid #1E293B" : "1px solid #E5E7EB") : "none",
                }}
              >
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <Avatar name={res.employeeName} />
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 14 }}>{res.employeeName}</div>
                    <div style={{ fontSize: 11, color: "#9CA3AF" }}>Base: {fmt(res.baseSalary)}</div>
                  </div>
                </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ height: 4, background: isDark ? "#1E293B" : "#E5E7EB", borderRadius: 2, overflow: "hidden" }}>
          <div
            style={{
              height: "100%",
              background: "#3B82F6",
      ...
      <div style={{ padding: "24px 16px 0 16px" }}>
        ...
        {step === 1 && (
          <div>
            ...
            <p style={{ fontSize: 14, color: isDark ? "#9CA3AF" : "#6B7280", marginBottom: 24 }}>
                ...
                {employees.map((emp) => (
                  ...
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <div>
                        <div style={{ fontWeight: 700, fontSize: 14 }}>{emp.fullName}</div>
                        <div style={{ fontSize: 11, color: "#9CA3AF" }}>{emp.role || "Employee"}</div>
                      </div>
                    </div>
                    <input
                      type="checkbox"
                ...
                return (
                  ...
                  >
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
                      <Avatar name={emp.fullName} />
                      <div>
                        <div style={{ fontWeight: 700, fontSize: 14.5 }}>{emp.fullName}</div>
                        <div style={{ fontSize: 11, color: "#9CA3AF" }}>Base: {fmt(emp.monthlySalary)}</div>
                      </div>
                    </div>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          value={adj.leaveDays || ""}
                          placeholder="0"
                          onChange={(e) => handleAdjustmentChange(emp._id, "leaveDays", e.target.value)}
                          style={{
                            width: "100%",
                            padding: "8px 12px",
                            background: isDark ? "#090D16" : "#F3F4F6",
                      ...
                      <div>
                        ...
                        <input
                          ...
                          placeholder="0"
                          onChange={(e) => handleAdjustmentChange(emp._id, "overtimeHours", e.target.value)}
                          style={{
                            width: "100%",
                            padding: "8px 12px",
                            background: isDark ? "#090D16" : "#F3F4F6",
                      ...
                      <div>
                        ...
                        <input
                          ...
                          value={adj.bonus || ""}
                          placeholder="0"
                          onChange={(e) => handleAdjustmentChange(emp._id, "bonus", e.target.value)}
                          style={{
                            width: "100%",
                            padding: "8px 12px",
                            background: isDark ? "#090D16" : "#F3F4F6",
        ...
        {step === 5 && (
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              >
                <span style={{ fontSize: 14, fontWeight: 500 }}>Included Employees</span>
              ...
              >
                <span style={{ fontSize: 14, fontWeight: 500 }}>Total Base Salary</span>
                <span style={{ fontSize: 18, fontWeight: 700 }}>{fmt(totalBase)}</span>
              </div>
              ...
              >
                <span style={{ fontSize: 14, fontWeight: 500 }}>Adjustments Logged</span>
        ...
        {step === 6 && (
          <div>
            ...
            <p style={{ fontSize: 14, color: isDark ? "#9CA3AF" : "#6B7280", marginBottom: 24 }}>
              Confirm and finalize payroll for {MONTHS.find((m) => m.value === selectedMonth)?.label} {selectedYear}.
            </p>
            ...
            >
              <h3 style={{ fontSize: 15, fontWeight: 700, color: "#EF4444", marginBottom: 6 }}>
                ⚠️ Important Notice
              </h3>
              <p style={{ fontSize: 13, color: isDark ? "#FCA5A5" : "#B91C1C", lineHeight: 1.5 }}>
                Finalizing payroll will commit transactions to the database and lock payroll calculations for this cycle. Ensure all adjustments are correct.
              </p>
            </div>
            ...
            >
              ...
              />
              <span style={{ fontSize: 13.5, fontWeight: 500, lineHeight: 1.4 }}>
                I confirm that the payroll data is correct and ready to be finalized.
              </span>
            </label>
          </div>
        )}
      ...
      >
        ...
        <button
          ...
          style={{
            flex: 2,
            padding: "12px",
            background: !isStepValid() || submitting ? (isDark ? "#1E293B" : "#E5E7EB") : "#3B82F6",
            color: !isStepValid() || submitting ? (isDark ? "#4B5563" : "#9CA3AF") : "white",
            border: "none",
            borderRadius: 10,
            fontSize: 14,
            fontWeight: 700,
