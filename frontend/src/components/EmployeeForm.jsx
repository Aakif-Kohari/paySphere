...
import { employeeValidationSchema, initialEmployeeValues } from '../validation/employeeSchema';
 ...
 */
const FormField = ({ label, name, type = 'text', placeholder, required, as = 'input', children }) => (
 ...
 */
export default function EmployeeForm({ initialValues, onSubmit, onCancel, isSubmitting, isEdit }) {
    const mergedInitialValues = {
        ...
        joiningDate: initialValues?.joiningDate ? new Date(initialValues.joiningDate).toISOString().split('T')[0] : '',
    };
            ...
            onSubmit={async (values, { setSubmitting, setErrors }) => {
                try {
                    ...
                    const cleanedValues = {
                        ...
                        overtimeRate: values.overtimeRate === '' ? 0 : Number(values.overtimeRate),
                        monthlySalary: Number(values.monthlySalary),
                        dateOfBirth: values.dateOfBirth === '' ? null : new Date(values.dateOfBirth).toISOString(),
                        joiningDate: values.joiningDate === '' ? null : new Date(values.joiningDate).toISOString(),
                        bankDetails: {
                ...
                <Form className="space-y-8 bg-white dark:bg-slate-800 p-6 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-sm">
                    ...
                    <fieldset className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <FormField label="Full Name" name="fullName" placeholder="e.g. Jane Doe" required />
                    ...
                    <fieldset className="space-y-6 border-t border-gray-200 dark:border-slate-700 pt-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <FormField label="Role / Designation" name="role" placeholder="e.g. Software Engineer" required />
                            <FormField label="Department" name="department" placeholder="e.g. Engineering" />
                    ...
                    <fieldset className="space-y-6 border-t border-gray-200 dark:border-slate-700 pt-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <FormField label="Monthly Salary" name="monthlySalary" type="number" placeholder="e.g. 50000" required />
                            <FormField label="Overtime Rate (per hour)" name="overtimeRate" type="number" placeholder="e.g. 250" />
                        </div>
                    </fieldset>
                    ...
                    <fieldset className="space-y-6 border-t border-gray-200 dark:border-slate-700 pt-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <FormField label="Bank Name" name="bankDetails.bankName" placeholder="e.g. HDFC Bank" />
                            <FormField label="Account Number" name="bankDetails.accountNumber" placeholder="e.g. 1234567890" />
                            <FormField label="Routing / IFSC Code" name="bankDetails.routingCode" placeholder="e.g. HDFC0001234" />
                        </div>
                    </fieldset>
