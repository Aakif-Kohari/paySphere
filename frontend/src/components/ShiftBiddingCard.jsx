            ...
            <div className="flex items-center justify-between">
                ...
                {shift.premiumMultiplier > 1 && (
                    <span className="flex items-center gap-1 text-xs font-bold text-green-600 dark:text-green-400">
                        <AttachMoneyIcon fontSize="small" /> {shift.premiumMultiplier}x Pay
                    </span>
                )}
            </div>

            <div>
                <h3 className="text-lg font-bold text-gray-900 dark:text-white">{dateStr}</h3>
                ...
                {shift.requiredDepartment && (
                    <div className="flex items-center gap-2 mt-1 text-sm text-gray-600 dark:text-slate-400">
                        ...
                        <span>{shift.requiredDepartment} {shift.requiredRole ? `• ${shift.requiredRole}` : ''}</span>
                    </div>
                )}
            </div>
